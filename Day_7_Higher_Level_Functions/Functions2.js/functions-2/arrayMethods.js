...
//const postTaxPrices = prices.map(/* ProvideYour Callback Here );
...
let myStrongest = monstersInYourPocket.filter((MonsterObj)=>MonsterObj.CP>200).filter((MonsterObj)=>MonsterObj.monster)
...
const addTax=orders.map((priceObj)=>priceObj.price*priceObj.tax).map((newPrice)=>newPrice.toFixed(2))
...
const totalOrder=purchases.map((priceObj)=>priceObj.price).reduce((currentTotal,purchase)=>currentTotal + purchase)
